...
the model selected. The models are described in parts:
...
consensus bases acording to RepBase and Simple Repeats from TRF.
  c) Transitional frecuencies for fixed windows in GC% classes.

To create the new sequence first we create a new non-repetitive sequence. 
Then, the elements are "bombarded" in random positions.

The main idea is to recreate a fake sequence generator without limits of 
size and sequences as real as the intergenic regions of a genome.
...
Required parameters:
  ...
  -n --name        Output files to create [*.fasta and *.inserts]
...
Optional or automatic parameters:
  ...
  --repeats_file   File with repeats info (generated by createFakeModel.pl)
  --gct_file       File with GC transitions info (generated by createFakeModel.pl)
  --kmer_file      File with k-mer info (generated by createFakeModel.pl)
...
pod2usage(-verbose => 2) unless (defined $model and defined $size);
...
$gct_file     = "$dir/$model/$model.GCt.W$win.data"         unless (defined $gct_file);
$kmer_file    = "$dir/$model/$model.kmer.K$kmer.W$win.data" unless (defined $kmer_file);
$repeat_file  = "$dir/$model/$model.repeats.W$win.data"     unless (defined $repeat_file);
$repbase_file = "$dir/RepBase/RepeatMaskerLib.embl"         unless (defined $repbase_file);
...
sub insertElements {
	...
	while ($repfra < $repthr) {
	    ...
	    print "selected: $new\n" if (defined $debug);
	    if ($new =~ m/SIMPLE/) {
			next if ( 0.999 > rand); # hack to avoid SIMPLE repeats high density
