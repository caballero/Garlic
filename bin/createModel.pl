    ...
    foreach my $file (@repeat) {
        ...
        while (<T>) {
            ...
            if ( exists $idHash{$id} )
            {
              ...
              my $pos = $repdata{$rid}{'ini'} + int(($end - $repdata{$rid}{'ini'})/2);
...
sub classGC {
    ...
    # Redefine @gc....note this is unecessary recalculated...but ok for testing.
    @gc = ();
    my $lastCut = -1;
...
sub calcBinGC {
    ...
    foreach my $i (0..$#cumulative) {
      ...
      if ($bucket>$previousbucket) {
        #print join("\t", $gcBuckets, $i<$#cumulative ? $i : 1000, $c), "\n";
