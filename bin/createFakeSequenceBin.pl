...
consensus bases acording to RepBase and Simple Repeats from TRF.
  c) Transitional GC binned frecuencies for fixed windows.

To create the new sequence, first we create a new non-repetitive sequence,
then, the artificially evolved repeat elements are "bombarded" in random 
positions and random orientations.

The main idea is to recreate an infinite fake sequence generator as real as
the intergenic regions of a genome.
...
Optional or automatic parameters:
  ...
  -t --type        Include only this type of repeats
...
my %insert       =    (); # Hash to store repeat inserts info
...
my $gct_file     = undef; 
my $repeat_file  = undef;
my $kmer_file    = undef;
my $repbase_file = undef;
my $insert_file  = undef;
...
# formatFasta => break a sequence in blocks (70 col/line default)
sub formatFasta {
	my $sseq  = shift @_;
	my $col   = shift @_;
	$col ||= 70;
